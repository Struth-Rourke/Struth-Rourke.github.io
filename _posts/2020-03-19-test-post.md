...
![Feature Importance BSM] /img/Screen Shot 2020-04-30 at 1.56.01 PM.png {: .center-block :}
...
which yielded R^2 = .9972; although somewhat concerning due to possible feature leakage, we can assume that the fitted model does 
indeed create a price that is consistent to what the BSM model would predict, as well as what the actual option trade is. In 
conclusion, we can speculate that this random foerst model does some calculation very similar to the BSM model to predict 
option prices.

![Feature Importance Other] /img/Screen Shot 2020-04-30 at 1.56.36 PM.png {: .center-block :}

Surprisingly, just using the trade delta and ..... the accuracy for the random forest model is high with an R^2 = .7000. This 
insiuates that additional analysis should be done looking into the specific effect that each option greek has on the trade price and whether or not they alone can accurately predict the actual price with continued accuracy. 
...
permuated the dataset values and ran an eli5 feature importance graph for the two best linear models and both random forests, 
the below visualizations speak to each model and the respective feature importances after permuting 


Permutation methodology is an important aspect of this for ........... why? thesis? output?
...
Bulletted topic list for continued research includes:
