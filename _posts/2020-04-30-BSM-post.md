...
Because the trade delta had such a high feature importance score, as well as the fact that the second random forest model, ran 
using the alternative data, produced such a high R^2 relative to the Linear Regression model, I created a plot that compares 
the trade delta and trade price, with two marginal graphs along the x and y axis. I also included a regression trendline 
(in green) to identify the R^2 of just the trade delta on the trade price. It produced an R^2 of 0.87 -- far better than what 
the linear model fitting was able to produce before. One thing to consider with this is the total amount of data that was used 
to create this plot.

You can interact with this plot by clicking the link below labeled: "Delta Plot".

[Delta Plot](/delta_plot.html)
